...
def router(source_df, lim: int, feature_file1: str, feature_file2: str, feature_file3: str, elec_util_file: str, medical_file: str, water_file: str) -> None:
    total_rows = 0
    no_ufokn_id = 0
    far_count = 0
    elec_util_count = 0
    ...
    for index, row in source_df.iterrows():
        ...
        # elif key == 'amenity' and value == 'hospital':
            ...
            if total_rows <= 0.33 * len(source_df.index):
                write_feature(new_feature, feature_file1)
            elif total_rows <= 0.67 * len(source_df.index):
                write_feature(new_feature, feature_file2)
            else:
                write_feature(new_feature,feature_file3)
            far_count += 1
        # elif extract_address_info(total_rows)[2].lower() == 'cincinnati':
        #     new_feature = far_proc(total_rows, Counter(used_uids))
        #     if total_rows <= 0.5 * len(source_df.index):
        #         write_feature(new_feature, feature_file1)
        #     else:
        #         write_feature(new_feature, feature_file2)
        #     far_count += 1
...
if __name__ == '__main__':
    FAR_file1 = 'data_FAR-1.ttl'
    if os.path.exists(FAR_file1):
        os.remove(FAR_file1)
    write_name_spaces(FAR_file1)
    write_props(FAR_file1)

    FAR_file2 = 'data_FAR-2.ttl'
    if os.path.exists(FAR_file2):
        os.remove(FAR_file2)
    write_name_spaces(FAR_file2)
    write_props(FAR_file2)

    FAR_file3 = 'data_FAR-3.ttl'
    if os.path.exists(FAR_file3):
        os.remove(FAR_file3)
    write_name_spaces(FAR_file3)
    write_props(FAR_file3)
    ...
    router(df, -1, FAR_file1, FAR_file2, FAR_file3, ELEC_file, MED_file, WATER_file)  # set limit to -1 to parse entire parquet file
